...
/*   Updated: 2026/01/21 09:06:51 by pberne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parse_test(char *finename)
{
	...
	ft_lstadd_back(&(v3i_node->data_lst), ft_lstnew_gc_id(int_node,
			malloc_id_parsing));
	double_node = ft_malloc_id(sizeof(t_parsing_data), malloc_id_parsing);
	double_node->type = parsing_double;
	...
	ft_lstadd_back(&lst, ft_lstnew_gc_id(double_node, malloc_id_parsing));
	while (line)
	{
		ft_str_replace(line, ",\t\n", ' ');
