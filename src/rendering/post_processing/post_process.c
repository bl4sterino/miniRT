...
void	ft_set_and_pack(t_data *d)
{
	...
	err = clSetKernelArg(d->opencl.kernel_set_and_pack, 0, sizeof(cl_mem),
			&d->opencl.a);
	if (err != CL_SUCCESS)
	{
	...
	err = clEnqueueNDRangeKernel(d->opencl.command_queue,
			d->opencl.kernel_set_and_pack, 1, 0, &size, 0, 0, 0, 0);
...
void	ft_accumulate_and_pack(t_data *d)
{
	...
	err = clSetKernelArg(d->opencl.kernel_accumulate_and_pack, 0,
			sizeof(cl_mem), &d->opencl.a);
	d->frame_count += 1.0;
	coef = (float)(1.0 / d->frame_count);
	err += clSetKernelArg(d->opencl.kernel_accumulate_and_pack, 3,
			sizeof(float), &coef);
	if (err != CL_SUCCESS)
	{
	...
	size = WIDTH_WIN * HEIGHT_WIN;
	err = clEnqueueNDRangeKernel(d->opencl.command_queue,
			d->opencl.kernel_accumulate_and_pack, 1, 0, &size, 0, 0, 0, 0);
	clFinish(d->opencl.command_queue);
}

void	ft_blur_kernel(t_data *d, cl_kernel blur_kernel, int radius,
		int spacing)
{
	int		err;
	...
	if (err != CL_SUCCESS)
		ft_printf("Err blur exec; %s\n", get_cl_error(err));
	err = clSetKernelArg(blur_kernel, 4, sizeof(cl_mem),
			&d->opencl.gaussian_mat);
	if (err != CL_SUCCESS)
		ft_printf("Err blur exec; %s\n", get_cl_error(err));
	err = clEnqueueNDRangeKernel(d->opencl.command_queue, blur_kernel, 2, 0,
			(void *)(&size), 0, 0, 0, 0);
	if (err != CL_SUCCESS)
		ft_printf("Err blur exec; %s\n", get_cl_error(err));
	clFinish(d->opencl.command_queue);
...
void	ft_update_gaussian_mat(t_data *d, int radius)
{
	...
	d->opencl.gaussian_mat = clCreateBuffer(d->opencl.context,
			CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, size * sizeof(float),
			gaussian_mat, &err);
	if (err != CL_SUCCESS)
	{
		free(gaussian_mat);
...
void	ft_blur(t_data *d)
{
	...
	if (d->denoise)
	{
		...
		err += clEnqueueWriteBuffer(d->opencl.command_queue,
				d->opencl.positions_buff, CL_TRUE, 0, sizeof(t_v3f)
				* SCREEN_SIZE, d->image.positions, 0, 0, 0);
		if (err != CL_SUCCESS)
			ft_printf("Error write normals to gpu\n");
		blur_distance_fallof = 3.0f / 3.0f;
		inv_sigma_sq = 1.0f / (2.0f * blur_distance_fallof
				* blur_distance_fallof);
		err = clSetKernelArg(d->opencl.kernel_blur_h, 7, sizeof(float),
				&inv_sigma_sq);
		if (err != CL_SUCCESS)
			ft_printf("Err blur exec; %s\n", get_cl_error(err));
		err = clSetKernelArg(d->opencl.kernel_blur_v, 7, sizeof(float),
				&inv_sigma_sq);
		if (err != CL_SUCCESS)
			ft_printf("Err blur exec; %s\n", get_cl_error(err));
		radius = 10;
...
void	ft_process_normals(t_data *d)
{
	...
	size = WIDTH_WIN * HEIGHT_WIN;
	err = clEnqueueNDRangeKernel(d->opencl.command_queue,
			d->opencl.kernel_process_normals, 1, 0, &size, 0, 0, 0, 0);
	if (err != CL_SUCCESS)
...
void	ft_post_process(t_data *d)
{
	...
	{
		err = clEnqueueWriteBuffer(d->opencl.command_queue, d->opencl.a,
				CL_TRUE, 0, sizeof(t_v3f) * SCREEN_SIZE, d->image.normals, 0, 0,
				0);
		if (err != CL_SUCCESS)
	...
	{
		err = clEnqueueWriteBuffer(d->opencl.command_queue, d->opencl.a,
				CL_TRUE, 0, sizeof(t_v3f) * SCREEN_SIZE, d->image.current_frame,
				0, 0, 0);
		if (err != CL_SUCCESS)
	...
	err = clEnqueueReadBuffer(d->opencl.command_queue,
			d->opencl.out_packed_buff, CL_TRUE, 0, HEIGHT_WIN * WIDTH_WIN
			* sizeof(int), d->image.addr, 0, 0, 0);
	clFinish(d->opencl.command_queue);
}
