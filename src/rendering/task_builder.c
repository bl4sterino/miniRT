...
/*   Updated: 2026/03/03 11:52:32 by pberne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_tasks_tile(t_data *d)
{
	...
	d->threads_data.tasks_total_count = (WIDTH_WIN / TASK_TILE_SIZE) * (HEIGHT_WIN
		/ TASK_TILE_SIZE);
...
{
	d->threads_data.tasks_total_count = SCREEN_SIZE / TASK_LINE_SIZE + (SCREEN_SIZE % TASK_LINE_SIZE != 0);
	d->threads_data.tasks = ft_malloc(sizeof(t_render_task)
			* d->threads_data.tasks_total_count);
	int i = 0;
